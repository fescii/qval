...
const topUsersLoggedIn = /*sql*/`
  ...
  SELECT u.hash, u.name, u.email, u.bio, u.picture, u.followers, u.following, u.stories, u.replies, u.verified, COALESCE(uv.views_count, 0) AS user_views, COALESCE(uf.is_following, FALSE) AS is_following
...
const topUsersLoggedOut = /*sql*/`
  WITH user_views AS (SELECT author, COUNT(*) AS views_count FROM story.views WHERE "createdAt" > :daysAgo GROUP BY author)
  SELECT u.hash, u.name, u.email, u.bio, u.picture, u.followers, u.following, u.stories, u.replies, u.verified, COALESCE(uv.views_count, 0) AS user_views, FALSE AS is_following
  FROM account.users u
  LEFT JOIN  user_views uv ON u.hash = uv.author
  ORDER BY user_views DESC, u.followers DESC, u.stories DESC, u.replies DESC
  LIMIT :limit OFFSET :offset;

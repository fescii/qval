...
const topTopicsLoggedIn = /* sql */ `
  ...
  topic_sections_json AS (SELECT topic, JSON_AGG(JSON_BUILD_OBJECT('id', id, 'content', content, 'order', "order", 'title', title) ORDER BY "order") AS sections FROM topic.sections GROUP BY topic)
